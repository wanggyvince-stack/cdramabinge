...
export default function middleware(request: NextRequest) {
  ...
  // Redirect all non-English locale paths to English equivalent (301)
  // Consolidates vi/th/id → en for de-indexing
  const localeMatch = pathname.match(/^\/(vi|th|id)(\/.*)?$/);
  if (localeMatch) {
    const url = request.nextUrl.clone();
    url.pathname = `/en${localeMatch[2] || ''}`;
    return NextResponse.redirect(url, 301);
  }
